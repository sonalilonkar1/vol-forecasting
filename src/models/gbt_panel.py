...
# Defaults aligned with HAR baseline (TFT-ready features)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        "Gradient-Boosted Trees (HAR-panel default) with strict t-1 features and YAML splits"
    )
    p.add_argument(
        "--source",
        type=Path,
        ...
        help=(
            ...
            "data/processed/features.parquet (same as HAR)."
        ),
    )
    p.add_argument(
        "--horizons",
...
def fill_missing_features(tab: pd.DataFrame, feature_cols: list[str]) -> pd.DataFrame:
    """Forward/back-fill features per asset to keep rows aligned with returns panel."""
    if not feature_cols:
        return tab
...
def build_gbt_table(
    ...
    """
    Prepare a table for GBT regression at a given horizon using HAR-style rows.
    """
    tab = df.copy()
    ...
    exclude = {"date", "asset", "ticker", "_row_id"}
    for h in (1, 5, 22):
    ...
    tab = fill_missing_features(tab, features)
...
def main():
    ...
    if cfg.get("assets"):
        ...
        if matched:
            ...
            if missing:
                ...
                print(f"[GBT-HAR] Warning: missing assets in source: {joined}. Using available subset only.")
        else:
            joined = ", ".join(sorted(requested))
            print(f"[GBT-HAR] Warning: requested assets ({joined}) not found in source. Using all available assets instead.")
    ...
    for H in args.horizons:
        ...
        out = args.out_dir / f"gbt_har_panel_h{H}.csv"
